...
void IC_Init(void) {
    ...
    TIM_TimeBaseInitStructure.TIM_Prescaler = 72 - 1; /* 预分频器设为35。 */
...
/*    TIM_ICInit(TIM3, &TIM_ICInitStructure);
    ...
    TIM_ICInitStructure.TIM_Channel = TIM_Channel_2; *//* 选择TIM3的通道1。 *//*
    TIM_ICInitStructure.TIM_ICFilter = 0xF; *//* 输入滤波器设为0xF。 *//*
    TIM_ICInitStructure.TIM_ICPolarity = TIM_ICPolarity_Falling; *//* 输入极性设为上升沿。 *//*
    TIM_ICInitStructure.TIM_ICPrescaler = TIM_ICPSC_DIV1; *//* 输入捕获预分频器设为不分频。 *//*
    TIM_ICInitStructure.TIM_ICSelection = TIM_ICSelection_IndirectTI; *//* 输入捕获信号选择为直接TI。 *//*
    TIM_ICInit(TIM3, &TIM_ICInitStructure);*/
...
uint32_t IC_GetDuty(void) {
    return (TIM3->CCR2 + 1) * 100 / TIM3->CCR1;
}