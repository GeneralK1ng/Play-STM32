...
void PWM_Init(void) {
    ...
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1; // 配置 PIN1
    ...
    TIM_TimeBaseInitStructure.TIM_Period = 20000 - 1;    // 设置定时器周期，定时器最大值为10000，即 ARR
    TIM_TimeBaseInitStructure.TIM_Prescaler = 72 - 1;    // 设置预分频器，将系统时钟分频到7200Hz，即 PSC
